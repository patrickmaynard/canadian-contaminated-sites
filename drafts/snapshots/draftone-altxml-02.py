...
def importSites(mode,connection,cursor):
	...
	for site in root[1]:
		...
		importdata[identifier]['SiteStatus_EN'] = retrieveSiteStatus(site)
		...
		sql += 'Location_Longitude, Location_Country_EN, Name_EN, SiteStatus_EN' 
		sql += ') '
		sql += ' VALUES("'
		...
		sql += importdata[identifier]['SiteStatus_EN']
		sql +='");'
		...
		for sitestatus in site.iter('SiteStatus'):
			...
			for english in sitestatus.iter('EN'):
				try:
					...
					output = '000 - english site status not present in XML'
		return(output)
