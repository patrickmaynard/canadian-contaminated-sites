...
def importOrgs(mode,connection,cursor):
	...
	importdata = {}
	for org in root[0]:
		identifier = retrieveOrgCode(org) 
		importdata[identifier] = {}
		importdata[identifier]['code'] = retrieveOrgCode(org)
		importdata[identifier]['org_en'] = retrieveOrgEn(org)
		importdata[identifier]['org_fr'] = retrieveOrgFr(org)
		print importdata[identifier]['org_fr']
		...
		sql += ') '
		sql += ' VALUES("'
		sql += importdata[identifier]['code']+'","'
		sql += importdata[identifier]['org_en']+'","'
		sql += importdata[identifier]['org_fr']
		sql +='");'
...
def importSites(mode,connection,cursor):
	...
	for site in root[1]:
		...
		sql += 'Location_Longitude, Location_Country_EN, Name_EN, SiteStatus_Status_EN, SiteStatus_Description_EN' 
		sql += ') '
		sql += ' VALUES("'
		sql += importdata[identifier]['FederalSiteIdentifier']+'","'
		sql += importdata[identifier]['ReportingOrganization']+'","'
		sql += importdata[identifier]['Created']+'","'
		sql += importdata[identifier]['LastModified']+'","'
		sql += importdata[identifier]['Location_Latitude']+'","'
		sql += importdata[identifier]['Location_Longitude']+'","'
		sql += importdata[identifier]['Location_Country_EN']+'","'
		sql += importdata[identifier]['Name_EN']+'","'
		sql += importdata[identifier]['SiteStatus_Status_EN']+'","' 
		sql += importdata[identifier]['SiteStatus_Description_EN']
		sql +='");'
...
def dumpSitesForCompare(connection,cursor):
	#The dumpSitesForCompare() function takes in a connection and a cursor.
	#The connection should be a sqlite3 connection object pointing at the relevant database.
	#The cursor should be a sqlite3 cursor object. 
...
def initSitesTable(connection):
	...
	cursor.execute('''CREATE TABLE sites 
		       (FederalSiteIdentifier TEXT, ReportingOrganization TEXT, Created TEXT, LastModified TEXT, Location_Latitude TEXT, Location_Longitude TEXT, Location_Country_EN TEXT, Name_EN TEXT, SiteStatus_Status_EN, SiteStatus_Description_EN TEXT)''')
	connection.commit()
	return(cursor)
...
def createConnection(mode):	
	...
	if mode == 'test':
		...
		print 'Note: This import can take a long time -- up to ______ -- to complete.'
...
dumpSitesForCompare(connection,cursor)
