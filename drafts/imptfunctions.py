...
def importOrgs(mode,connection,cursor):
	...
	for org in root[0]:
		...
		print '(As of the time when this script was drafted in early 2016, that total was 26 orgs for the test XML and the same number for the live XML.)'
...
def importSites(mode,connection,cursor):
	...
	for site in root[1]:
		...
		sql += 'Location_Longitude, Location_Province, Location_Municipality,Location_Country_EN, PopulationCount_KM1, PopulationCount_KM5, PopulationCount_KM10, PopulationCount_KM25, PopulationCount_KM50, Name_EN, SiteStatus_Status_EN, SiteStatus_Description_EN, Classification_Code' 
		sql += ') '
		sql += ' VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '
...
def initSitesTable(connection):
	...
	cursor.execute('''CREATE TABLE sites 
		       (FederalSiteIdentifier TEXT, ReportingOrganization TEXT, Created TEXT, LastModified TEXT, Location_Latitude TEXT, Location_Longitude TEXT, Location_Province TEXT, Location_Municipality TEXT, Location_Country_EN TEXT, PopulationCount_KM1, PopulationCount_KM5, PopulationCount_KM10, PopulationCount_KM25, PopulationCount_KM50, Name_EN TEXT, SiteStatus_Status_EN, Classification_Code TEXT,  SiteStatus_Description_EN TEXT)''')
	connection.commit()
	return(cursor)
